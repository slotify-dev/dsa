import { bubble } from '../../src/sorting';
import { describe, expect, it } from 'bun:test';
import { copyArray } from '../../src/utils/helper';
...
describe('Bubble Sort', () => {
  ...
  describe('Object arrays with custom comparator', () => {
    ...
    it('should sort objects by age', () => {
      ...
      const result = bubble(arr, (a, b) => a.age - b.age);
      expect(result).toEqual([
        { name: 'Jane', age: 20 },
        { name: 'Alice', age: 22 },
        { name: 'John', age: 25 },
    ...
    it('should sort objects by name', () => {
      const arr = copyArray(people);
      const result = bubble(arr, (a, b) => a.name.localeCompare(b.name));
      expect(result).toEqual([
        { name: 'Alice', age: 22 },
        { name: 'Bob', age: 30 },
        { name: 'Jane', age: 20 },
  ...
  describe('Stability', () => {
    it('should maintain the relative order of equal elements (stability)', () => {
      ...
      const result = bubble(arr, (a, b) => a.value - b.value);
  ...
  // Test optimization (early termination)
  describe('Optimization', () => {
    it('should terminate early when the array becomes sorted', () => {
      // This is hard to test directly, but we can check that it works correctly
      const arr = [2, 1, 3, 4, 5];
      const result = bubble(arr);
      expect(result).toEqual([1, 2, 3, 4, 5]);
      expect(result).toBe(arr);
